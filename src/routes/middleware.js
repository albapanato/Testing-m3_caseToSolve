...
const log = (req, res, next) => {
  console.log(
    "Datos almacenados en el fichero (./logs/registroPeticiones.log) "
  );
  ...
  res.on("finish", () => {
    const logText = `${dayjs().format()} | ${req.body.email || "N/A"} | ${
      req.method
    } | ${req.url} | ${res.statusCode} | ${res.statusMessage}\n`;
    fs.appendFileSync("./logs/registroPeticiones.log", logText);
  });
  res.on("error", (err) => {
    const errorText = `${dayjs().format()} | ${req.body.email || "N/A"} | ${
      req.method
...
const checkToken = async (req, res, next) => {
  ...
  if (!req.headers["authorization"]) {
    return res
      ...
      .json({
        error:
          "Necesitas un codigo de autorizacion o tener acceso a este portal",
      });
  }
    ...
    if (payload.expiredAt < dayjs().unix() || !req.user.userAccess) {
      ...
      if (payload.expiredAt < dayjs().unix()) {
        return res
          .status(403)
          .json({
            error: "TU CODIGO HA CADUCADO, VUELVE A INICIALIZAR SESION",
          });
      } else {
...
const verifyIsAdmin = async (req, res, next) => {
  if (!req.headers["authorization"]) {
    return res
      ...
      .json({
        error:
          "Necesitas un codigo de autorizacion para proceder con la solicitud",
      });
  }
  ...
  try {
    ...
    if (infoUser.userRol === "user" || !infoUser.userRol) {
      const mensaje = res
        .status(403)
        .json({
          error: "No tienes acceso para borrar ningun tipo de informacion",
        });
      res.send(mensaje);
    }
    next();
    ...
    if (
      ...
      if (payload.expiredAt < dayjs().unix()) {
        ...
        return res
          .status(403)
          .json({
            error: "NO TIENES ACCESO a modificar los datos de este restaurante",
          });
      }
    }
